...
func ExchangeV3AtForTest(s V3AuthService, ctx context.Context, input V3ExchangeInput, now time.Time) (string, int64, error) {
	return ExchangeV3WithClockForTest(s, ctx, input, func() time.Time { return now })
}

// ExchangeV3WithClockForTest 允许请求在等待数据库或 Redis 时推进校验时钟。
func ExchangeV3WithClockForTest(s V3AuthService, ctx context.Context, input V3ExchangeInput, now func() time.Time) (string, int64, error) {
	return s.(*v3AuthService).exchangeWithClock(ctx, input, now)
}
