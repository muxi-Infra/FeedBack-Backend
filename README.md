...
**🎯 功能说明**: 检测服务运行状态、系统资源使用情况和性能指标，用于监控和运维。
...
```text
{
    ...
    "data": {
        "status": "ok",                    // 🟢 服务整体状态
        "response_ms": 1,                  // ⚡ 响应时间（毫秒）
        "system": {
            "cpu_percent": 10.30,          // 💻 系统CPU使用率（%）
            "memory_total": 15556,         // 💾 系统总内存（MB）
            "memory_used": 13710,          // 💾 系统已用内存（MB）
            "memory_percent": 88,          // 💾 系统内存使用率（%）
            "disk_total": 274,             // 💿 磁盘总空间（GB）
            "disk_used": 193,              // 💿 磁盘已用空间（GB）
            "disk_percent": 70.59          // 💿 磁盘使用率（%）
        },
        "process": {
            "cpu_percent": 0.63,           // ⚙️ 进程CPU使用率（%）
            "memory_rss_mb": 31,           // 🧠 进程物理内存（MB）
            "goroutines": 3,               // 🔄 Goroutine数量
            "go_heap_alloc_mb": 9          // 🗂️ Go堆内存分配（MB）
        },
    }
}
```
